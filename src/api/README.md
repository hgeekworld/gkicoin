...
API default service port is `5959`.  However, if running the desktop or standalone releases from the website, the port is randomized by default.
...
```sh
curl http://127.0.0.1:5959/api/v1/csrf
```
...
```sh
curl http://127.0.0.1:5959/api/v1/health
```
...
```sh
curl http://127.0.0.1:5959/api/v1/version
```
...
```sh
curl http://127.0.0.1:5959/api/v1/balance\?addrs\=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,nu7eSpT6hr5P21uzw7bnbxm83B6ywSjHdq
```
...
```sh
curl http://127.0.0.1:5959/api/v1/outputs?addrs=6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```

or

```sh
curl http://127.0.0.1:5959/api/v1/outputs?hashes=7669ff7350d2c70a88093431a7b30d3e69dda2319dcb048aa80fa0d19e12ebe0
```
...
Parses and validates a gkicoin address. Returns the address version in the response.
...
```sh
curl -X POST http://127.0.0.1:5959/api/v2/address/verify \
...
```sh
curl http://127.0.0.1:5959/api/v1/wallet?id=2017_11_25_e5fb.wlt
```
...
```sh
curl http://127.0.0.1:5959/api/v1/wallet/transactions?id=2017_11_25_e5fb.wlt
```
...
```sh
curl http://127.0.0.1:5959/api/v1/wallets
```
...
```sh
curl http://127.0.0.1:5959/api/v1/wallets/folderName
```
...
```json
{
    "address": "/Users/user/.gkicoin/wallets"
}
```
...
```sh
curl http://127.0.0.1:5959/api/v1/wallet/newSeed
```
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/create \
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/newAddress \
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/update \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=$id' \
 -d 'label=$label'
```
...
```sh
curl http://127.0.0.1:5959/api/v1/wallet/balance?id=2018_03_07_3088.wlt
```
...
```sh
curl -X POST  http://127.0.0.1:5959/api/v1/wallet/spend \
  -H 'Content-Type: application/x-www-form-urlencoded' \
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/transaction -H 'content-type: application/json' -d '{
    "hours_selection": {
        "type": "auto",
        "mode": "share",
        "share_factor": "0.5"
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/unload \
 -H 'Content-Type: x-www-form-urlencoded' \
 -d 'id=2017_05_09_d554.wlt'
```
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/encrypt \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/decrypt \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/wallet/seed \
 -H 'Content-type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl http://127.0.0.1:5959/api/v1/pendingTxs
```
...
```sh
curl http://127.0.0.1:5959/api/v1/transaction?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
```sh
curl http://127.0.0.1:5959/api/v1/rawtx?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
```sh
curl -X POST http://127.0.0.1:5959/api/v1/injectTransaction -H 'content-type: application/json' -d '{
...
```sh
curl http://127.0.0.1:5959/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY&confirmed=1
```

To get address related unconfirmed transactions:
```sh
curl http://127.0.0.1:5959/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY&confirmed=0
```

To get all addresses related transactions:

```sh
curl http://127.0.0.1:5959/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```
...
```sh
curl http://127.0.0.1:5959/api/v1/resendUnconfirmedTxns
```
...
```sh
curl -X POST -H 'Content-Type: application/json' http://127.0.0.1:5959/api/v2/transaction/verify \
...
```sh
curl http://127.0.0.1:5959/api/v1/blockchain/metadata
```
...
```sh
curl http://127.0.0.1:5959/api/v1/blockchain/progress
```
...
```sh
curl http://127.0.0.1:5959/api/v1/block?hash=6eafd13ab6823223b714246b32c984b56e0043412950faf17defdbb2cbf3fe30
```

or

```sh
curl http://127.0.0.1:5959/api/v1/block?seq=2760
```
...
```sh
curl http://127.0.0.1:5959/api/v1/blocks?start=1&end=2
```
...
```sh
curl http://127.0.0.1:5959/api/v1/last_blocks?num=2
```
...
```sh
curl http://127.0.0.1:5959/api/v1/explorer/address?address=2NfNKsaGJEndpSajJ6TsKJfsdDjW2gFsjXg
```
...
```sh
curl http://127.0.0.1:5959/api/v1/uxout?uxid=8b64d9b058e10472b9457fd2d05a1d89cbbbd78ce1d97b16587d43379271bed1
```
...
```sh
curl http://127.0.0.1:5959/api/v1/address_uxouts?address=6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```
...
```sh
curl http://127.0.0.1:5959/api/v1/coinSupply
```
...
```sh
curl "http://127.0.0.1:5959/api/v1/richlist?n=4&include-distribution=true"
```
...
```sh
curl "http://127.0.0.1:5959/api/v1/addresscount"
```
...
```sh
curl 'http://127.0.0.1:5959/api/v1/network/connection?addr=176.9.84.75:6000'
```
...
```sh
curl 'http://127.0.0.1:5959/api/v1/network/connections'
```
...
```sh
curl 'http://127.0.0.1:5959/api/v1/network/defaultConnections'
```
...
```sh
curl 'http://127.0.0.1:5959/api/v1/network/connections/trust'
```
...
```sh
curl 'http://127.0.0.1:5959/api/v1/network/connections/exchange'
```
