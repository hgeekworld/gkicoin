...
import (
	...
	"hgeekl.com/gkicoin/src/daemon/gnet"
	"hgeekl.com/gkicoin/src/daemon/pex"
	"hgeekl.com/gkicoin/src/util/iputil"
	"hgeekl.com/gkicoin/src/util/utc"
)
...
func (gpm *GivePeersMessage) GetPeers() []string {
	...
	idx := 0
	for _, ipaddr := range gpm.Peers {
		_, port, err := iputil.SplitAddr(ipaddr.String())
		if err != nil || port != GkicoinPort {
			logger.Debugf("IP failed  %s failed", ipaddr.String())
		} else {
			peers[idx] = ipaddr.String()
			idx += 1
		}
	}
	return  peers[:idx]
