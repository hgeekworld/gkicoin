...
// CreateAddresses genCount addresses deterministically from seed.  coinType is either CoinTypeBitcoin or CoinTypeGkicoin.
...
func CreateAddresses(coinType CoinType, seed string, genCount int, hideSecretKey bool) (*ReadableWallet, error) {
	...
	for _, sec := range seckeys {
		...
		case CoinTypeGkicoin:
			entry = GetGkicoinWalletEntry(pub, sec)
		default:
...
// GetGkicoinWalletEntry returns a ReadableEntry in Gkicoin format
func GetGkicoinWalletEntry(pub cipher.PubKey, sec cipher.SecKey) ReadableEntry {
