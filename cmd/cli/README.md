...
Gkicoin command line interface

The CLI command APIs can be used directly from a Go application, see [Gkicoin CLI Godoc](https://godoc.org/hgeekl.com/gkicoinsrc/cli).
...
```bash
$ cd $GOPATH/src/hgeekl.com/gkicoincmd/cli
$ ./install.sh
```
...
```bash
$ PROG=gkicoin-cli source $GOPATH/src/hgeekl.com/gkicoin/cmd/cli/autocomplete/bash_autocomplete
```
...
CLI will connect to gkicoin node rpc address `http://127.0.0.1:6420` by default.
...
The default CLI wallet dir is located in `$HOME/.gkicoin/wallets/`, change it by setting the
...
The default CLI wallet file name is `gkicoin_cli.wlt`, change it by setting the `WALLET_NAME` env.
...
After the installation, you can run `gkicoin-cli` to see the usage:

```
$ gkicoin-cli

NAME:
   gkicoin-cli - the gkicoin command line interface

USAGE:
   gkicoin-cli [global options] command [command options] [arguments...]
...
COMMANDS:
     ...
     addressGen            Generate gkicoin or bitcoin addresses
     ...
     send                  Send gkicoin from a wallet or an address to a recipient address
     showConfig            show cli configuration
     status                Check the status of current gkicoin node
     transaction           Show detail info of specific transaction
     verifyAddress         Verify a gkicoin address
     version
     walletBalance         Check the balance of a wallet
     walletDir             Displays wallet folder address
     walletHistory         Display the transaction history of specific wallet. Requires gkicoin node rpc.
...
ENVIRONMENT VARIABLES:
    ...
    COIN: Name of the coin. Default "gkicoin"
...
### Add Private Key
Add a private key to a gkicoin wallet.

```bash
$ gkicoin-cli addPrivateKey [command options] [private key]
```

```
OPTIONS:
    -f value [wallet file or path] private key will be added to this wallet
    if not specified then default wallet ($HOME/.gkicoin/wallets//wallets/gkicoin_cli.wlt)
    will be used
```

#### Example
```bash
$ gkicoin-cli addPrivateKey -f $WALLET_PATH $PRIVATE_KEY
```
...
```bash
$ gkicoin-cli addressBalance [addresses]
```

#### Example
```bash
$ gkicoin-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
Generate new gkicoin or bitcoin addresses.

```bash
$ gkicoin-cli addressGen [command options] [arguments...]
```
...
OPTIONS:
        ...
        --bitcoin, -b            Output the addresses as bitcoin addresses instead of gkicoin addresses
...
#### Examples
##### Generate `n` number of gkicoin addresses
```bash
$ gkicoin-cli addressGen --count 2
```
...
```bash
$ gkicoin-cli addressGen --count 2 --bitcoin
```
...
```bash
$ gkicoin-cli addressGen --count 2 --hide-secret
```
...
```bash
$ gkicoin-cli addressGen --count 2 --only-addr
```
...
```bash
$ gkicoin-cli addressGen --count 2 --seed "my super secret seed"
```
...
```bash
gkicoin-cli addressGen --count 2 --hex
```
...
```bash
$ gkicoin-cli addressOutputs [address list]
```

#### Example
```bash
gkicoin-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ gkicoin-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ gkicoin-cli blocks 41 42
```
...
Checks if the given database file contains valid gkicoin blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ gkicoin-cli checkdb [db path]
```

#### Example
```bash
$ gkicoin-cli checkdb $DB_PATH
```
...
```bash
$ gkicoin-cli createRawTransaction [command options] [to address] [amount]
```
...
```bash
$ gkicoin-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to a specific change address

```bash
$ gkicoin-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ gkicoin-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
```bash
$ gkicoin-cli createRawTransaction -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ gkicoin-cli decodeRawTransaction [raw transaction]
```

Decode a raw gkicoin transaction.

#### Example

```bash
gkicoin-cli decodeRawTransaction 
```
...
Broadcast a raw gkicoin transaction.
Output is the transaction id.

```bash
$ gkicoin-cli broadcastTransaction [raw transaction]
```

```bash
$ gkicoin-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Generate a wallet
Generate a new gkicoin wallet.

```bash
$ gkicoin-cli generateWallet [command options]
```
...
OPTIONS:
        ...
        -f value  [walletName] Name of wallet. The final format will be "yourName.wlt".
                             If no wallet name is specified a generic name will be selected. (default: "gkicoin_cli.wlt")
...
```bash
$ gkicoin-cli generateWallet -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ gkicoin-cli generateWallet -n 2
```
...
```bash
$ gkicoin-cli generateWallet -f "secret_wallet.wlt"
```
...
```bash
$ gkicoin-cli generateWallet -l "cli wallet"
```
...
Generate new addresses for a gkicoin wallet.

```bash
$ gkicoin-cli generateAddresses [command options]
```

```
OPTIONS:
        -n value    [numberOfAddresses]  Number of addresses to generate (default: 1)
        -f value    [wallet file or path] Generate addresses in the wallet (default: $HOME/.gkicoin/wallets//wallets/gkicoin_cli.wlt)
        --json, -j  Returns the results in JSON format
```

##### Generate an address for the default wallet
```bash
$ gkicoin-cli generateAddresses
```
...
```bash
$ gkicoin-cli generateAddresses $WALLET_PATH
```
...
```bash
$ gkicoin-cli generateAddresses -n 2
```
...
##### Generate a JSON output
```bash
$ gkicoin-cli generateAddresses --json
```
...
### Last blocks
Show the last `n` gkicoin blocks.
By default the last block is shown.

```bash
$  gkicoin-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ gkicoin-cli lastBlocks
```
...
List addresses in a gkicoin wallet.

```bash
$ gkicoin-cli listAddresses [walletName]
```

If no `walletName` is given then default wallet ($HOME/.gkicoin/wallets/gkicoin_cli.wlt) is used.

> NOTE: The wallet name `gkicoin_cli.wlt` or full path `$HOME/.gkicoin/wallets/gkicoin_cli.wlt` can be used.
        When only the wallet name is given then the default wallet dir, $HOME/.$COIN/wallets is used.

#### Examples
##### List addresses of default wallet
```bash
$ gkicoin-cli listAddresses
```
...
```bash
$ gkicoin-cli listAddresses $WALLET_NAME or $WALLET_PATH
```
...
### List wallets
List wallets in the gkicoin wallet directory.

```bash
$ gkicoin-cli listWallets
```
...
### Send
Make a gkicoin transaction.

```bash
$ gkicoin-cli send [command options] [to address] [amount]
```

```
OPTIONS:
        -f value    [wallet file or path] From wallet. If no path is specified your default wallet
                    (`$HOME/.gkicoin/wallets/gkicoin_cli.wlt`) path will be used.
        -a value    [address] From address
...
```bash
$ gkicoin-cli send $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific wallet
```bash
$ gkicoin-cli send -f $WALLET_PATH $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ gkicoin-cli send -f $WALLET_PATH -a $FROM_ADDRRESS $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ gkicoin-cli send -f $WALLET_PATH -a $FROM_ADDRESS -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending to multiple addresses
```bash
$ gkicoin-cli send -f $WALLET_PATH -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
##### Generate a JSON output
```bash
$ gkicoin-cli send -f $WALLET_PATH -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```json
{
    "wallet_directory": "/home/user/.gkicoin/wallets",
    "wallet_name": "gkicoin_cli.wlt",
    "data_directory": "/home/user/.gkicoin",
    "coin": "gkicoin",
...
```bash
$ gkicoin-cli transaction [transaction id]
```

#### Example
```bash
$ gkicoin-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
### Verify address
Verify whether a given address is a valid gkicoin addres or not.

```bash
$  gkicoin-cli verifyAddress [gkicoin address]
```
...
```bash
$ gkicoin-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ gkicoin-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
```bash
$ gkicoin-cli verifyAddress 21YPg
```
...
Check the wallet a gkicoin wallet.

```bash
$ gkicoin-cli walletBalance [wallet]
```

> NOTE: Both the full wallet path or only the wallet name can be used.
        If no wallet is specified then the default wallet: `$HOME/.$COIN/wallets/gkicoin_cli.wlt` is used.

#### Example
##### Balance of default wallet
```bash
$ gkicoin-cli walletBalance
```
...
```bash
$ gkicoin-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ gkicoin-cli walletBalance ~/.gkicoin/wallets/2018_04_01_198c.wlt
```
...
Get the current gkicoin wallet directory.

```bash
$ gkicoin-cli walletDir [command options]
```
...
```bash
$ gkicoin-cli walletHistory [command options]
```
...
##### Default wallet
```bash
$ gkicoin-cli walletHistory
```

##### Specific wallet
```bash
$ gkicoin-cli walletHistory -f $WALLET_NAME
```
*OR*
```bash
$ gkicoin-cli walletHistory -f $WALLET_PATH
```
...
```bash
$ gkicoin-cli walletOutputs [wallet file]
```

#### Examples
##### Default wallet
```bash
$ gkicoin-cli walletOutputs
```

##### Specific wallet
```bash
$ gkicoin-cli walletHistory $WALLET_NAME
```
*OR*
```bash
$ gkicoin-cli walletHistory $WALLET_PATH
```
...
### CLI version
Get version of current gkicoin cli.

```bash
$ gkicoin-cli version [command options]
```
...
```bash
$ gkicoin-cli send $RECIPIENT_ADDRESS $AMOUNT -c $CHANGE_ADDRESS
```
...
```bash
$ gkicoin-cli send -c $CHANGE_ADDRESS $RECIPIENT_ADDRESS $AMOUNT
```
